...
use stwo_the_end::models::{PlayerState, StoryNodeMeta, Choice, PlayerDecision, Decision, StoryCompleted, GamblingLevelConfig, Chances, Multiplier};
...
#[dojo::contract]
mod actions {
    use super::{ITextNovelGame, PlayerState, StoryNodeMeta, Choice, PlayerDecision, Decision, StoryCompleted, GamblingLevelConfig};
    ...
    impl GameImpl of ITextNovelGame<ContractState> {
        fn start_new_game(ref self: ContractState) {
            ...
            //let first_storynode = StoryNodeMeta{ id: 1, text: 'You wake up in a forest.', is_ending: false };
            world.write_model(@StoryNodeMeta{ id: 1, text: 'You wake up in a forest.', is_ending: false });
            ...
            world.write_model(@StoryNodeMeta { id: 11, text: 'You find a river.', is_ending: true });
            world.write_model(@StoryNodeMeta { id: 12, text: 'You meet a wolf.', is_ending: true });
            world.write_model(@PlayerState { player, current_node: 1, story_completed: false});
            // let config = get_level_config(1);
            // world.write_model(@GamblingLevelConfig { level: config.level, multiplier: config.multiplier, chances: config.chances});
        }
...
//         },
//         5 => StaticGamblingConfig {
//             level: 5,
//             multiplier: Multiplier::Huge,
//             chances: Chances::Little,
//         },
//         _ => None, // fallback for unsupported levels
//     }
// }
